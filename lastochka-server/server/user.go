...
func replyCreateUser(s *Session, msg *ClientComMessage, rec *auth.Rec) {
	...
	for i := range creds {
		...
		// Keep normalized credential values and generate restricted tags server-side.
		if globals.validators[cr.Method].addToTags && tag != "" {
			if prefix, value := validateTag(tag); prefix == cr.Method && value != "" {
				cr.Value = value
				credTags = append(credTags, tag)
			}
		}
	}
...
func buildProfileSearchTags(public any) []string {
