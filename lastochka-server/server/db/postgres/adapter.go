...
func (a *adapter) UserCredExists(method, value string) (bool, error) {
	ctx, cancel := a.getContext()
	if cancel != nil {
		defer cancel()
	}
	...
	err := a.db.QueryRow(ctx,
		"SELECT EXISTS(SELECT 1 FROM credentials WHERE synthetic=$1 OR synthetic LIKE '%:'||$2)",
		synth, synth).Scan(&exists)
	if err != nil {
		return false, err
	}
	return exists, nil
